...
const HeroAvatar: React.FC<{ hero: any }> = ({ hero }) => {
  // 从HEROES数据中获取英雄详细信息
...
const MobileHeroAvatar: React.FC<{ hero: any }> = ({ hero }) => {
  // 从HEROES数据中获取英雄详细信息
  const heroData = HEROES.find((h) => h.id === hero.id);
  const avatarUrl = heroData ? getHeroAvatarUrl(heroData) : null;
  const displayName = heroData ? heroData.cnName || heroData.name : hero.name;
  ...
  const DesktopLayout = () => (
    <div
      className={`group hover:bg-gray-700/30 transition-all duration-200 ${
        index !== totalRooms - 1 ? "border-b border-gray-700" : ""
      }`}
    >
      <div className="p-4">
              ...
              <div className="flex items-center gap-3 mt-3">
                ...
                {room.selected_heroes && room.selected_heroes.length > 0 && (
                  <div className="flex items-center gap-1">
                    {room.selected_heroes.slice(0, 10).map((hero: any) => (
          ...
          {room.selected_heroes && room.selected_heroes.length > 0 && (
            <div className="flex items-center gap-1">
              {room.selected_heroes.slice(0, 8).map((hero: any) => (
                <MobileHeroAvatar key={hero.id} hero={hero} />
              ))}
