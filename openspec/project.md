...
- **国际化**: 自研 i18n 系统，支持中英双语切换，语言包预加载优化
...
- **国际化架构**: 基于 React Context 的 i18n 服务，支持嵌套翻译键和参数化
- **语言持久化**: localStorage 保存用户语言偏好，跨标签页自动同步
...
### i18n 架构说明
- **核心服务**: `/i18n/services/i18n.service.ts` - 单例模式管理语言切换和翻译
- **React集成**: `/i18n/components/I18nProvider.tsx` - Context Provider 提供全局翻译功能
- **语言包位置**: `/public/i18n/locales/` - JSON 格式的翻译文件
- **性能优化**: 语言包预加载（中文用 preload，英文用 prefetch）

### 多语言开发规范

#### 新功能开发流程
1. **文本国际化**: 所有用户可见的文本必须使用翻译函数
   ```typescript
   // 错误示例
   <button>确认</button>
...
2. **翻译键命名规范**
   - 使用层级结构: `模块.组件.功能.具体文本`
   - 示例: `ui.roomList.filter.searchPlaceholder`
   - 避免过深的嵌套（建议不超过3层）

3. **添加新翻译**
   - 同时更新 `/public/i18n/locales/zh-CN.json` 和 `/public/i18n/locales/en-US.json`
   - 保持键名完全一致
   - 英文翻译要语义准确，避免机器翻译
...
4. **参数化翻译**
   ```typescript
   // 带参数的翻译
   t('hero.selectedCount', { count: selectedCount })

   // 语言包定义
   {
     "hero.selectedCount": "已选择 {{count}} 个英雄"
   }
   ```
...
#### 性能注意事项
- 语言包已启用预加载，新功能避免过度增加语言包大小
- 复杂组件使用 React.memo 优化重渲染
- 翻译函数使用 useCallback 避免不必要的函数创建
