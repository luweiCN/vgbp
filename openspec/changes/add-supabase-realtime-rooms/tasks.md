...
- [ ] 1.2 配置数据库表结构（users, rooms, room_participants, bp_states）
- [ ] 1.3 设置 Row Level Security (RLS) 策略
...
## 4. 实时同步功能
- [ ] 4.1 配置 Supabase Realtime 订阅
- [ ] 4.2 实现 BP 状态实时同步 Hook
- [ ] 4.3 创建实时状态更新组件
- [ ] 4.4 实现冲突检测和解决机制
- [ ] 4.5 添加同步状态指示器
- [ ] 4.6 实现离线状态检测和处理
...
- [ ] 7.4 优化头部按钮布局和状态管理
  - [ ] 7.4.1 未登录状态：显示「注册登录」「本地模式」「加入房间」按钮
  - [ ] 7.4.2 已登录状态：显示「创建房间」「本地模式」「加入房间」「用户信息」「登出」
  - [ ] 7.4.3 移除模式切换开关，改为直接按钮功能
- [ ] 7.5 更新 BP 界面支持权限控制
- [ ] 7.6 添加实时状态指示器
- [ ] 7.7 创建房间邀请和分享界面
- [ ] 7.8 优化移动端响应式设计
