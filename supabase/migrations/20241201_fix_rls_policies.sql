-- 创建一个 SQL 函数来执行 RLS 策略修复
CREATE OR REPLACE FUNCTION fix_rls_policies()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    -- 删除可能存在的冲突策略
    DROP POLICY IF EXISTS "Users can insert their own rooms" ON rooms;
    DROP POLICY IF EXISTS "Users can view public rooms" ON rooms;
    DROP POLICY IF EXISTS "Users can update their own rooms" ON rooms;
    DROP POLICY IF EXISTS "Users can delete their own rooms" ON rooms;

    -- 允许已认证用户创建房间
    CREATE POLICY "Users can insert their own rooms" ON rooms
    FOR INSERT
    WITH CHECK (auth.uid() = owner_id);

    -- 允许已认证用户查看所有公开房间
    CREATE POLICY "Users can view public rooms" ON rooms
    FOR SELECT
    USING (is_public = true OR owner_id = auth.uid());

    -- 允许房主更新自己的房间
    CREATE POLICY "Users can update their own rooms" ON rooms
    FOR UPDATE
    USING (auth.uid() = owner_id);

    -- 允许房主删除自己的房间
    CREATE POLICY "Users can delete their own rooms" ON rooms
    FOR DELETE
    USING (auth.uid() = owner_id);

    -- 修复 room_participants 表的 RLS 策略
    DROP POLICY IF EXISTS "Users can insert room participants for their rooms" ON room_participants;
    DROP POLICY IF EXISTS "Users can view room participants" ON room_participants;

    CREATE POLICY "Users can insert room participants for their rooms" ON room_participants
    FOR INSERT
    WITH CHECK (
      room_id IN (
        SELECT id FROM rooms WHERE owner_id = auth.uid()
      ) OR user_id = auth.uid()
    );

    CREATE POLICY "Users can view room participants" ON room_participants
    FOR SELECT
    USING (
      room_id IN (
        SELECT id FROM rooms WHERE owner_id = auth.uid()
      ) OR user_id = auth.uid()
    );

    -- 修复 room_settings 表的 RLS 策略
    DROP POLICY IF EXISTS "Users can manage room settings for their rooms" ON room_settings;

    CREATE POLICY "Users can manage room settings for their rooms" ON room_settings
    FOR ALL
    USING (
      room_id IN (
        SELECT id FROM rooms WHERE owner_id = auth.uid()
      )
    );
END;
$$;

-- 立即执行修复
SELECT fix_rls_policies();